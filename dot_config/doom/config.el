...
;; ── WSL / fish 适配（由配置会话追加；fish 不是 POSIX shell）──
;; Doom doctor 会因为 $SHELL=fish 报警：Emacs 内部调用 shell 的地方需要 POSIX shell
(setq shell-file-name (executable-find "bash"))
(setq shell-command-switch "-c")
;; 交互式终端模拟器（vterm / M-x shell）仍使用 fish，符合日常习惯
(setq-default vterm-shell "/usr/bin/fish")
(setq-default explicit-shell-file-name "/usr/bin/fish")

;; ── 键位（WSL 追加）──────────────────────────────────────────
;; 1) insert 模式下连按 jk 退出到 normal（等价 nvim 里的 jk -> <Esc>）
;; 2) leader 不用配：Doom 默认 leader 就是 SPC；localleader 是 SPC m（可用 C-h v doom-leader-key 查看）
(after! evil
